...
IconLib is a small Bash script that can do that. To use it, clone the repository or download the ZIP file, open the Terminal, `cd` to the directory of the repository, and then run:

```
./iconlib -f applist.txt
```
...
- The first time the `iconlib` script is run, macOS will refuse to perform the required operations and show a notification warning that 'Terminal' has been prevented from modifying apps on your Mac. Click on the `Allow...` button to be redirected to `System Settings` > `Privacy & Security` > `App Management` and toggle the switch next to the `Terminal`. Run the script again and this time it should work as expected.
